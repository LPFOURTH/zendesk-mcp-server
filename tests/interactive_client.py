...
TOOL_HELP = {
    "list_tickets": {
        ...
        "params": {"page": "int?", "per_page": "int? (1-100)", "sort_by": "str?", "sort_order": "asc|desc?"},
        "example": '{"per_page": 5}',
    },
    "get_ticket": {
    ...
    "update_article": {
        ...
        "params": {"id": "int (required)", "title": "str?", "body": "str? (HTML)", "draft": "bool?"},
    ...
    "search": {
        ...
        "params": {
            ...
            "sort_by": "str?", "sort_order": "asc|desc?",
            "page": "int?", "per_page": "int? (1-100)",
        },
    ...
    "create_release_note": {
        ...
        "example": '{"markdown_content": "### Functionality 1 Name\\nFeature\\n### Functionality 1 Description\\nDesc", "section_id": 123}',
    },
}
...
class MCPSession:
    ...
    def connect(self):
        ...
        if not self._connected.wait(timeout=30):
            raise TimeoutError("SSE connection timed out (30s). Server may be cold-starting — try again.")

    def _sse_loop(self):
        while True:
            try:
                if USE_SSL:
                    conn = http.client.HTTPSConnection(HOST, PORT, context=SSL_CTX, timeout=300)
                else:
        ...
        if not self.endpoint:
            for line in self._buf.split("\n"):
                ...
                if stripped.startswith("data:") and "sessionId=" in stripped:
                    ...
                    try:
                        self.session_id = self.endpoint.split("sessionId=")[1].split("&")[0]
                    except IndexError:
    ...
    def call(self, method, params=None, timeout=60):
        ...
        payload = json.dumps({"jsonrpc": "2.0", "id": rid, "method": method, "params": params or {}}).encode()
...
def main():
    print(f"\n{'='*60}")
    print(f"  Zendesk MCP Interactive Client")
    ...
    while True:
        ...
        if tool_name not in TOOL_HELP:
            print(f"  \033[33mUnknown: '{tool_name}'. Type 'tools' for the list.\033[0m\n")
            continue
        ...
        if len(parts) > 1:
            try:
                ...
                print(f"  \033[2mExample: {tool_name} {TOOL_HELP[tool_name]['example']}\033[0m\n")
                continue
