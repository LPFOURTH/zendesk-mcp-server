# IT Self-Help Agent Instructions
...
- incident or service_request + eit_general → `eit_general_subcategory`
...
For all L3 and L4 fields, use the exact option values listed in the `create_it_ticket` tool schema (human-readable names like `adobe_acrobat`, `laptop`, `phishing`).

**eit_general_subcategory** (when either category = eit_general):
...
- `access_request` → `access_request_type`: `network_drive` | `security_group` | `web_portal` | `other`
