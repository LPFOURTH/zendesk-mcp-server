...
| `/mcp/dev` (dormant Arch F) | Microsoft Entra OAuth 2.1 + PKCE | Each individual Fourth user | Entra JWT validated against Microsoft JWKS |
...
| # | Layer | What it protects against | How we apply it | Why it matters | Status |
|---|---|---|---|---|---|
| 1 | **Door check** | Outsiders calling the MCP | Entra single-tenant config / Zendesk OAuth on every request; HTTPS only | Without it, any internet caller could invoke our write tools | ✅ Live |
| 2 | **Record everything** | Untraceable misuse | Structured JSON log per tool call → Azure Log Analytics (90-day retention); 5-tile Azure Workbook + alert rules to Teams | If something bad happens we need to know who, what, when — for forensics, compliance, and trust | ⚠️ Partial (Zendesk side ✓, MCP side scheduled PR #4) |
